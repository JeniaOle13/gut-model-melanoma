...
tumor_mean <- tumor.df[tumor.df$time_point < 18,] %>%  
      group_by(group, time_point, batch) %>% 
      summarise(mean = mean(tumor_size), sd = sd(tumor_size))

tumor_plot <- ggplot()+
    geom_line(tumor.df[tumor.df$time_point < 18,], 
              mapping = aes(time_point, tumor_size, group = subject_id, col = group), alpha = 0.25)+
    geom_line(tumor_mean, mapping = aes(time_point, mean, col = group))+
...
#| message: false
#| warning: false
fm <- lmerTest::lmer(tumor_size ~ time_point + group + (1|batch) + (1|subject_id), data=tumor.df)
...
#| warning: false
#| include: false
qplot_tumor <- qplot(residuals(fm)) + 
      xlab("Residuals") + 
      ylab("Density") + 
      theme_bw()
```
...
#| message: false
#| warning: false
#| fig-cap: Figure 2. Distribution of residuals from the tumor size linear mixed model.
qplot_tumor
```
...
```{r fig.width=15, fig.height=5}
#| echo: false
#| fig-cap: Figure 4. Genus distribution per experimental samples stratified by group and replicate.
genus_barplot
```

#### Alpha diversity
<p style="text-align: justify;">
Alpha diversity was assessed using the Shannon index, calculated with the `diversity` function from the R `vegan` package. Statistical analysis revealed a significant reduction in Shannon index values in the M_LAC group compared to the M group (adj. p = 0.03). No other inter group differences in alpha diversity were observed. Complete results are presented in Table 5 and visualized in Figure 5.
<p>
...
```{r fig.width=7, fig.height=3}
#| echo: false
#| fig-cap: Figure 5. Shannon diversity index distribution stratified by group and replicate.
shannon_boxplot
```
...
#| message: false
#| warning: false
lmer.shannon <- lmer(shannon ~ time_point + group + (1|batch), data=alpha.df)
...
#| warning: false
#| include: false
qplot_alpha <- qplot(residuals(lmer.shannon)) + 
      xlab("Residuals") + 
      ylab("Density") + 
      theme_bw()
```
...
```{r fig.width=6, fig.height=5}
#| echo: false
#| fig-cap: Figure 7. Bi-dimentional visualization of microbiome strcutrure at genus level using non-metric multidimentional scaling and Bray-Curtis dissimilarity.
mds_plot
```
...
#| message: false
#| warning: false
adonis2(otu_table ~ batch + time_point + group, data = meta_data, permutations = 9999, by = "margin")
```

#### Differential abundance testing
<p style="text-align: justify;">
Differential abundance analysis was performed using `MaAsLin2` with default parameters. Compared to the M group (melanoma-only controls), the M_BIF group showed significant upregulation of CAG-594, *Caprobacter*, *Lachnotalea*, and *Klebsiella* genera, while *Muricomes*, *CAG-180*, *Intestinimonas*, *Holdemania*, and *Zag111* were downregulated. In the M_LAC group, *Klebsiella* was upregulated relative to controls, while multiple genera including *CAG-594*, *Lawsonibacter*, *UBA7182*, *Blautia*, *UBA3282*, *Paramuribaculum*, *Phocaeicola*, *Dorea_B*, *Flavonifractor*, *Dubosiella*, *Mailhella*, *UMGS1872*, *UBA3263*, *Lachnoclostridium*, and *UBA7057* were downregulated. Direct comparison between M_BIF and M_LAC groups revealed distinct microbial signatures: *CAG-594*, *Blautia*, *Paramuribaculum*, *UBA7057*, and *Marseille-P3106* were associated with M_BIF, whereas *Akkermansia*, *CAG-180*, and *Zag111* were linked to M_LAC. Complete results are presented in Tables 7–9 and Figures 8–10.
<p>
...
datatable(res.df_BIF, 
          caption = "Table 7. Differences between M_BIF and M groups.",
          style = "bootstrap5", 
          extensions = 'Buttons', 
          options = list(dom = 'lBfrtip',
                         buttons = c('csv', 'excel')))
...
```{r fig.width=6, fig.height=5}
#| echo: false
#| fig-cap: Figure 8. Differences between M_BIF and M groups.
volcano_BIF
```
...
```{r fig.width=6, fig.height=5}
#| echo: false
#| fig-cap: Figure 9. Differences between M_LAC and M groups.
volcano_LAC
```
...
```{r fig.width=6, fig.height=5}
#| echo: false
#| fig-cap: Figure 10. Differences between M_BIF and M_LAC groups.
volcano_BIF_LAC
```
